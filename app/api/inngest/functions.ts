    ...
    while (currentDay < totalDays) {
      ...
      if (recentJobs.length > 0) {
        await step.run("send-email", async () => {
          ...
          <h3 style="margin: 5px 0;">${job.Company.name}</h3>
          ...
          await resend.emails.send({
            ...
            to: ["ashebin24@gmail.com"],
            subject: "Check out recent job listings",
            html: `
            <div style='font-family:Arial, sans-serif; max-width: 600px; margin: 0 auto;'>
            <h2>Latest Job Opportunities</h2>
              ${jobListingHtml}
              <div style="margin-top:30px; text-align:center;">
              <a href="${process.env.BASE_URL}" style="background-color:#007bff; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px;">
              View More JObs
              </a>
              </div>
            </div>`,
